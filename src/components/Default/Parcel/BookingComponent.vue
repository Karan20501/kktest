<template>
  <div class="flex flex-col p-5 mt-4 bg-white rounded-lg md:w-96">
    <div v-for="(input, index) in deliveryStops" :key="index" class="mb-2">
      ...
      <div class="" v-if="index == 0">
        <label for="from" class="space-x-2 text-xl font-medium text-gray-700">
          <div class="flex justify-between">
            <label
              class="block mb-2 text-sm font-light text-gray-500 dark:text-white"
              >{{ $t("parcel.pickup") }}</label
            >
            <button
              v-on:click="toggleModl()"
              type="button"
              class="text-sm font-light text-green-500 hover:underline"
            >
              {{ $t("addAddress") }}
            </button>
          </div>
      ...
      <div v-if="index > 0">
        <label
          for="to"
          class="block mb-2 text-sm font-light text-gray-500 dark:text-white"
          >{{ $t("parcel.dropoffs") }}</label
        >
        <div class="flex items-center input wrapper">
          <select
            id="to"
            v-model="deliveryStops[index]"
            class="bg-gray-50 h-12 border border-gray-300 text-gray-500 text-sm rounded-lg block w-full p-2.5"
          >
            <option selected>{{ $t("parcel.pickupLocation") }}</option>
            <option
          ...
          <button
            class="ml-1"
            v-if="deliveryStops.length > 2"
            @click="removeField(index, deliveryStops)"
          >
    ...
    <button
      class="mb-5 text-sm font-light text-right text-green-500 underline"
      @click="addField({}, deliveryStops)"
    >
      + {{ $t("parcel.addStops") }}
    </button>
    <div>
      <label
        for="package"
        class="block mb-2 text-sm font-light text-gray-500 dark:text-white"
        >{{ $t("parcel.packageTypes") }}</label
      >
      <select
        id="package"
        class="bg-gray-50 h-12 border border-gray-300 text-gray-500 text-sm rounded-lg block w-full p-2.5"
        @change="showCouriers(selectedPackage)"
    ...
    <div>
      ...
      >
        ...
        <option :value="courier" v-for="courier in couriers" :key="courier.id">
          {{ courier.name }}
        </option>
      </select>
    </div>
        ...
        <div class="py-6 lg:px-4">
          ...
          <div class="w-full pt-5">
              ...
              <Disclosure v-slot="{ open }">
                ...
                <DisclosurePanel class="px-4 pt-4 pb-2 text-sm text-gray-500">
                  ...
                  <form
                    ...
                    :key="index"
                  >
                    <h3 class="mt-1 mb-5 text-sm font-medium">
                      Contact Info({{ stop.name }})
                    </h3>
                    <!-- <hr> -->
                    <div class="grid grid-cols-2 gap-5">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          v-model="recipients[index].phone"
                        />
            ...
            <div class="p-2 mt-2" v-show="showSummary">
              <div class="flex justify-between">
                <div
                  class="flex flex-row w-64 p-2 space-x-2 border border-gray-100 rounded-md"
              ...
              <div
                class="grid grid-cols-1 gap-2 mt-2 md:grid-cols-3"
                v-for="selectedRecipient in recipients"
                :key="selectedRecipient"
              >
                <div
                  class="flex flex-row p-2 space-x-2 border border-gray-100 rounded-md"
                >
                  <div>
                    <p class="text-xs font-medium text-gray-500">
                      {{ $t("login.name") }}
              ...
              <div class="flex flex-col">
                <div class="flex flex-row w-full space-x-2">
                  <div
                    class="flex p-1 space-x-2 border border-gray-100 rounded-md flow-row"
                  >
                    <p class="text-xs font-medium text-gray-500">
                      {{ $t("parcel.weight") }}:
                    </p>
            ...
            <div class="p-2 mt-2" v-show="showPayment">
              <Payment
                :summaryP="summary"
                :selectedPackge="selectedPackage"
                :deliveryZtops="deliveryStops"
                :length="plength"
                :width="width"
                :height="height"
                :weight="weight"
    ...
    <div class="relative h-full p-4 md:w-5/12 md:h-auto">
      ...
      <div class="relative bg-white rounded-lg shadow">
        ...
        <div class="px-6 py-6 lg:px-8">
          ...
          <p class="p-2 bg-green-500 rounded-md" v-if="message">
            {{ message }}
          </p>
          <form class="space-y-6" action="#">
            <div>
              <label for="name" class="text-sm font-light text-gray-500 x-5"
                >Name your address</label
              >
              <input
                type="text"
                id="name"
            ...
            <div>
              <label for="" class="text-sm font-light text-gray-500 x-5"
...
export default {
  ...
  methods: {
    ...
    showCouriers(selectedPkge) {
      // console.log(selectedPkge)
      if (selectedPkge != null) {
        // this.$store.commit("loading", true);
        axios
          .get(this.base_url + "api/vendors?type=package", {
            params: {
    ...
    vendorServiceAddres(courier, address) {
      const countryAllowed = this.vendorServiceCountry(
        courier.countries,
        address.country ?? ""
      );
      const stateAllowed = this.vendorServiceState(
        courier.states,
        address.state ?? ""
      );
      const cityAllowed = this.vendorServiceCity(
        courier.cities,
        address.city ?? ""
      );
