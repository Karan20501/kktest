<template>
  <div class="container max-w-full mx-auto" v-if="vendors">
    ...
    <div
      v-else-if="$route.params.slug == 'taxi-booking' || $route.params.slug == 'taxi' || $route.params.slug.match(/ride.*/) || $route.params.slug.match(/map.*/) || $route.params.slug.match(/fleet.*/)"
    >
      <Taxi />
    </div>
    <div
      v-else-if="$route.params.slug.match(/service.*/) || $route.params.slug.match(/booking.*/)"
    >
      <HomeBanner />
      <CategoryHome :categories="categories" class="mx-auto " />
...
.vendor-box:hover {
  background-color: #ff0000; /* Change to your preferred hover color */
